...
PROMPT = (
    "Identify the medicine from its composition in the image and provide its compositions, scientific name, usage, "
...
def validate_uploaded_file(request):
    ...
    if "image" not in request.FILES:
        return (
            JsonResponse(
                {"error": "No image file provided", "success": False}, status=400
            ),
            False,
        )
    ...
    try:
        ...
        if result:
            try:
                ...
                return JsonResponse({"success": True, "data": medicine_info})
