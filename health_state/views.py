...
class HealthTemplateCategoriesView(APIView):
    ...
    def get(self, request):
        ...
        categories = HealthTemplate.objects.filter(
            is_active=True, 
...
class HealthRecordDetailView(APIView):
    ...
    def patch(self, request, pk):
        ...
        serializer = HealthRecordUpdateSerializer(record, data=request.data, partial=True)
        if serializer.is_valid():
            record = serializer.save()
            response_serializer = HealthRecordSerializer(record)
            return Response(response_serializer.data, status=status.HTTP_200_OK)
...
class HealthRecordLatestView(APIView):
    ...
    def get(self, request):
        ...
        template_ids = HealthRecord.objects.filter(user=user).values_list(
